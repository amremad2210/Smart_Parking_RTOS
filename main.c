...
{
    /* Use the specific macros from app_config.h for each task */
    xTaskCreate(vInputTask, "Input", 
                INPUT_TASK_STACK_SIZE, NULL, INPUT_TASK_PRIORITY, NULL);
    
    xTaskCreate(vGateControlTask, "GateCtrl", 
                GATE_TASK_STACK_SIZE, NULL, GATE_CONTROL_TASK_PRIORITY, NULL);
    
    xTaskCreate(vSafetyTask, "Safety", 
                SAFETY_TASK_STACK_SIZE, NULL, SAFETY_TASK_PRIORITY, NULL);
    
    xTaskCreate(vLEDTask, "LED", 
                LED_TASK_STACK_SIZE, NULL, LED_TASK_PRIORITY, NULL);
    
    xTaskCreate(vStatusTask, "Status", 
                STATUS_TASK_STACK_SIZE, NULL, STATUS_TASK_PRIORITY, NULL);
}
